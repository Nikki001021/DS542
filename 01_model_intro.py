...
if __name__ == "__main__":
    ...
    dataset = CIFAR10(
        "/projectnb/dl4ds/materials/datasets/CIFAR",
        download=True,
        ...
        for Model in [MLP, CNN]:
            ...
            if device == "cuda" and torch.cuda.device_count() > 1:
                model = nn.DataParallel(model).to(
                    device
                )  # Wrap model for multi-GPU support, move to GPU
            else:
                model.to(device)
            ...
            )  # Adjusted for DataParallel wrapped models
            total_time, avg_time_per_epoch = train_model(
                model, trainloader, nn.CrossEntropyLoss(), optimizer, device, model_name
            )
            device_label = (
                "Multi-GPU" if isinstance(model, nn.DataParallel) else device.upper()
            )
            results.append(
                {
                    "Model": model_name,
    ...
    for result in sorted_results:
        table.add_row(
            [
                result["Model"],
                result["Device"],
                f"{result['Total Time (seconds)']:.2f}",
                f"{result['Avg Time per Epoch']:.2f}",
            ]
        )
